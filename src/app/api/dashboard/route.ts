...
export async function GET() {
  try {
    ...
    // AI stats (placeholder - would be calculated from actual AI usage)
    const aiStats = {
      grantsAnalyzed: savedGrants.length * 50 + Math.floor(Math.random() * 500),
      matchesFound: savedGrants.length * 3 + Math.floor(Math.random() * 50),
      timeSaved: `~${Math.floor(savedGrants.length * 2 + workspaces.length * 5)}h`,
