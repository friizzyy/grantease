    ...
    <section className="relative py-24 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          {/* Left column - content */}
          <div>
            <span className="inline-block font-mono text-xs uppercase tracking-widest text-pulse-accent mb-4">
              Trusted Data
            </span>

            <h2 className="font-serif text-4xl md:text-5xl text-pulse-text mb-6 tracking-tight">
            ...
            <div className="space-y-4">
              {trustPoints.map((item) => (
                <div key={item} className="flex items-center gap-3 group">
                  ...
                  <span className="text-pulse-text-secondary group-hover:text-pulse-text transition-colors">
                    {item}
                  </span>
                </div>
              ))}
            ...
            {trustStats.map((stat, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="font-serif text-2xl text-pulse-text mb-1">{stat.value}</div>
                  <div className="text-xs text-pulse-text-tertiary">{stat.label}</div>
                </div>
              )
            })}
          </div>
