...
export function AnimatedStat({ value, label, className, delay = 0 }: AnimatedStatProps) {
  return (
    ...
    >
      <div className="font-serif text-display text-pulse-accent mb-2">
        {value}
      </div>
      <div className="text-sm text-pulse-text-secondary">{label}</div>
    </div>
  )
}
...
export function AnimatedCategoryBadge({ children, index = 0, onClick }: AnimatedCategoryBadgeProps) {
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center rounded-full px-3 py-1 text-sm border border-pulse-border text-pulse-text-secondary hover:border-pulse-accent/30 hover:text-pulse-accent hover:bg-pulse-accent/5 transition-colors"
    >
      {children}
    </button>
  )
